export type CrawledPage={url:string;title:string;text:string;links:string[];emails:string[]};
...
function emailsOf(html:string){const out=new Set<string>();const decoded=decodeHtml(html);const mailto=/href=["']mailto:([^"'?#\s>]+)/gi;let m:RegExpExecArray|null;while((m=mailto.exec(decoded))){const email=m[1].trim().toLowerCase();if(email)out.add(email);}const visible=decoded.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi)??[];for(const email of visible)out.add(email.toLowerCase());return [...out].slice(0,50);}
