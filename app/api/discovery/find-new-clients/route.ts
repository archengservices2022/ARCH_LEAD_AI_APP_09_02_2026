...
export async function POST(){try{const s=await getSettings();
 ...
 return Response.json({ok:true,partial:warnings.length>0,dryRun:s.dryRun,emailSending:false,summary:{researched,qualified,review,rejected,contactsVerified:(enrichment.enriched??enrichment.verified??enrichment.updated??0),newLeads:promotion.ready??0,existingLeadsLinked:promotion.linked??0,draftsPrepared:drafts.queued??0},warnings,stages:{engineering,software,enrichment,promotion,drafts},message:`AI client search ${warnings.length?"completed with warnings":"complete"}: ${researched} prospects researched, ${qualified} qualified, ${review} research-more/review, ${promotion.directVerified??0} direct and ${promotion.generalVerified??0} general contacts verified, ${drafts.queued??0} outreach drafts prepared. No email is sent by discovery itself.`});}catch(e){return Response.json({ok:false,emailSending:false,error:e instanceof Error?e.message:"AI client acquisition failed"},{status:500});}}
