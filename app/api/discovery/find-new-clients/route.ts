...
export async function POST(){try{const s=await getSettings();
 ...
 if(qualified>0&&contactsVerified===0)blockers.push("Qualified prospects exist but no contacts were verified: domain/contact enrichment is blocking outreach.");
 ...
 return Response.json({ok:healthy,partial:warnings.length>0,dryRun:s.dryRun,emailSending:false,health:{status:healthy?"GREEN":"RED",sendableWork:healthy,blockers},summary:{researched,qualified,review,rejected,contactsVerified,newLeads,existingLeadsLinked:promotion.linked??0,draftsPrepared},warnings,stages:{engineering,software,enrichment,promotion,drafts},message:healthy?`Client acquisition funnel GREEN: ${researched} researched, ${qualified} qualified, ${contactsVerified} contacts verified, ${draftsPrepared} drafts prepared.`:`Client acquisition funnel RED: ${blockers.join(" ")}`},{status:healthy?200:424});}catch(e){return Response.json({ok:false,emailSending:false,health:{status:"RED"},error:e instanceof Error?e.message:"AI client acquisition failed"},{status:500});}}
