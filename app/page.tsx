...
export default async function Home() {
  ...
  const recentLeads = await db.prepare(`SELECT company,division,service,ai_score,qualification_status FROM leads WHERE qualification_status='Qualified' AND email_verified=1 AND outreach_status='Ready for Review' AND date(created_at)=date('now') ORDER BY ai_score DESC, created_at DESC LIMIT 5`).all<{company:string;division:string;service:string|null;ai_score:number;qualification_status:string}>();
  ...
  return <Shell active="/" dryRun={settings.dryRun}>
    ...
    <div className="grid"><article className="panel"><div className="panelHead"><div><h2>Recent Qualified Clients</h2><p>Today's new verified prospects ready for outreach</p></div></div>{recentLeads.results.length?<div className="tableWrap"><table><thead><tr><th>Company</th><th>Division</th><th>Service</th><th>Score</th></tr></thead><tbody>{recentLeads.results.map((l,i)=><tr key={i}><td>{l.company}</td><td>{l.division}</td><td>{l.service||"—"}</td><td>{l.ai_score}</td></tr>)}</tbody></table></div>:<div className="empty">No new verified prospects ready for outreach today.</div>}</article>
