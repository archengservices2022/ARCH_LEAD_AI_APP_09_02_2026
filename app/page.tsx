...
export default async function Home() {
  ...
  const recentLeads = await db.prepare(`SELECT company,division,service,ai_score,qualification_status FROM leads WHERE qualification_status='Qualified' AND lower(COALESCE(outreach_status,'')) NOT IN ('contacted','sent','follow-up due','replied','interested','meeting requested','proposal opportunity','do not contact') AND lower(company) NOT IN ('usacustomstainless','continental fabricators and erectors, llc','generalmachineinc','autocad drawing','curtisswright.com') AND lower(company) NOT LIKE '%fabrication drawings%' AND lower(company) NOT LIKE '%send drawings%' AND lower(company) NOT LIKE '%proposal template%' AND lower(company) NOT LIKE '%rishabh pro engineering%' AND lower(COALESCE(contact_email,'')) NOT LIKE '%@wonit.ai' ORDER BY created_at DESC LIMIT 5`).all<{company:string;division:string;service:string|null;ai_score:number;qualification_status:string}>();
  ...
  return <Shell active="/" dryRun={settings.dryRun}>
    ...
    <div className="grid"><article className="panel"><div className="panelHead"><div><h2>Recent Qualified Clients</h2><p>New qualified client prospects that have not already been contacted</p></div></div>{recentLeads.results.length?<div className="tableWrap"><table><thead><tr><th>Company</th><th>Division</th><th>Service</th><th>Score</th></tr></thead><tbody>{recentLeads.results.map((l,i)=><tr key={i}><td>{l.company}</td><td>{l.division}</td><td>{l.service||"—"}</td><td>{l.ai_score}</td></tr>)}</tbody></table></div>:<div className="empty">No new qualified client prospects awaiting outreach.</div>}</article>
